...
function checkStudentAuth() {
  ...
  if (savedSection && gateCard && mainContent) {
    ...
    if (sessionFilterBar) sessionFilterBar.style.display = "none"; // Hide session filters when specific section is entered
...
window.viewAllSections = function() {
  ...
  if (sessionFilterBar) sessionFilterBar.style.display = "flex"; // Show session filters for All Sections view
      ...
      Object.values(entriesByName).forEach((matches) => {
        if (matches.length > 1) {
          ...
          conflictDetails.push(
            `⚠️ ${first.type} Conflict: "${first.name}" is double-booked in ${involvedSections} on ${first.dayName} (${first.timeSlot}).`
          );
...
function renderAdminSections() {
  ...
  sectionsData.forEach((sec, secIdx) => {
    html += `
      <div class="section-card" style="margin-bottom:20px; padding:18px; background:#1e293b; border-radius:12px; border:1px solid #334155;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
          <h3 style="color:#38bdf8; margin:0;">${sec.title || sec.code} (${sec.session})</h3>
    ...
    (sec.slots || []).forEach((slot, rowIdx) => {
      html += `<tr style="border-bottom:1px solid #334155;">
        <td style="padding:8px; font-weight:600; color:#94a3b8; min-width:110px;">${slot}</td>`;
      ...
      DAYS.forEach((_, colIdx) => {
        ...
        html += `
          <td style="padding:6px; min-width:140px; ${cellStyle}">
            ...
            ${isConflicted ? `<div style="display:inline-block; background-color:#eab308; color:#854d0e; font-size:0.75rem; font-weight:700; padding:2px 6px; border-radius:4px; margin-top:4px;">⚠️ CONFLICT</div>` : ''}
          </td>`;
      });
