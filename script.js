...
function renderNextClassCard() {
  ...
  Object.keys(sec.cells).forEach(key => {
    const [rStr, cStr] = key.split("-"); // row = slot index, col = day index
    if (parseInt(cStr, 10) !== dayIdx) return;
    const startMinutes = getSlotStartMinutes(sec.slots[parseInt(rStr, 10)]);
    if (startMinutes === null) return;
    const diff = startMinutes - minutesNow;
    if (diff < 0) return;
    if (!best || diff < best.diff) best = { diff, cell: sec.cells[key], slot: sec.slots[parseInt(rStr, 10)], dayIdx };
  });
...
function renderTeacherNextClassCard() {
  ...
  sectionsData.forEach(sec => {
    ...
    Object.keys(sec.cells).forEach(key => {
      ...
      const diff = startMinutes - minutesNow;
      if (diff < 0) return;
      if (!best || diff < best.diff) best = { diff, cell, slot: sec.slots[parseInt(rStr, 10)], dayIdx, section: sec.code || sec.title };
    });
  });
  if (!best) { container.innerHTML = `<div class="next-class-empty">No more classes today 🎉</div>`; return; }
  const h = Math.floor(best.diff / 60), m = best.diff % 60;
...
window.openTodayOverviewModal = function () {
  ...
  if (!items.length) { container.innerHTML = `<p style="color:var(--text-muted); padding:8px;">No classes scheduled today.</p>`; }
  else {
    items.sort((a, b) => a.startMin - b.startMin);
    container.innerHTML = items.map(it => {
      let status = "upcoming", label = "🟢 Upcoming";
    ...
    <div style="background:var(--card-bg); border:1px solid var(--border-color); border-radius:var(--radius-xl); box-shadow:var(--shadow-modal); max-width:420px; width:100%; padding:20px; max-height:92vh; overflow-y:auto;">
      ...
      <div class="auth-form">
        ...
        <div><label for="ce-teacher">Teacher (optional):</label><input type="text" id="ce-teacher" list="ce-teacher-suggestions" placeholder="e.g. J. Muyot"><datalist id="ce-teacher-suggestions"></datalist>
          <small style="display:block; margin-top:4px; font-size:0.7rem; color:var(--text-muted);">💡 Auto-fills based on who's taught this subject before.</small>
        </div>
...
window.commitClassEntry = function () {
  ...
  // Cover the whole requested range (which may be several hours) using as
  // many rows as it takes: reuse any existing row that starts exactly where
  // we currently are, otherwise create a new row (up to 1 hour, or shorter if
  // a later existing row would overlap) to fill the gap. This is what lets
  // one submission — e.g. 10:00 AM to 2:00 PM — automatically populate every
  // hourly row in between, instead of the admin repeating this once per hour.
  ...
  const rowsToFill = [];
  let cursor = startMin, guard = 0;
  while (cursor < endMin && guard++ < 200) {
    const rows = readRows();
    const match = rows.find(r => r.startMin === cursor && r.endMin <= endMin);
    if (match) { rowsToFill.push(match.tr); cursor = match.endMin; continue; }
    const nextExisting = rows.filter(r => r.startMin > cursor).sort((a, b) => a.startMin - b.startMin)[0];
    let chunkEnd = Math.min(cursor + 60, endMin);
    if (nextExisting) chunkEnd = Math.min(chunkEnd, nextExisting.startMin);
    if (chunkEnd <= cursor) break; // safety net, shouldn't normally trigger
    const newRow = window.addEditorRow();
    newRow.querySelector(".edit-slot-input").value = `${minutesToDisplay12(cursor)} - ${minutesToDisplay12(chunkEnd)}`;
    rowsToFill.push(newRow);
    cursor = chunkEnd;
  }

  // Keep every row in chronological order after inserting any new blocks.
  ...
  rowsToFill.forEach(tr => {
    const dayCell = tr.querySelectorAll(".edit-day-cell")[dayIdx];
    if (dayCell) {
