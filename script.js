...
function renderStudentTodayList() {
  ...
  container.innerHTML = items.map(it => {
    ...
    if (minutesNow >= it.startMin && minutesNow < it.endMin) { status = "ongoing"; label = "🔵 Ongoing"; }
    else if (minutesNow >= it.endMin) { status = "completed"; label = "⚪ Completed"; }
...
function renderNextClassCard() {
  ...
  container.innerHTML = `<div class="next-class-card"><div class="next-class-eyebrow">Next Class</div>
    ...
    <div class="next-class-meta">🕒 ${best.timeDisplay} &nbsp;•&nbsp; 📍 ${displayRoom(best.cell.room, dayIdx)} &nbsp;•&nbsp; 👤 ${best.cell.professor || "—"}</div>
...
function renderTeacherNextClassCard() {
  ...
  container.innerHTML = `<div class="next-class-card"><div class="next-class-eyebrow">Next Class</div>
    <div class="next-class-subject">${best.cell.subject || best.cell.name || "—"}</div>
    <div class="next-class-meta">🕒 ${best.timeDisplay} &nbsp;•&nbsp; 📍 ${displayRoom(best.cell.room, dayIdx)} &nbsp;•&nbsp; 🏷️ Sec: ${best.section}</div>
    <div class="next-class-countdown">Starts in <span class="num">${h > 0 ? `${h}h ${m}m` : `${m}m`}</span></div></div>`;
}
